...
static uint32_t m_twi_scl_pin = 0;
static uint32_t m_twi_sda_pin = 0;
...
static bool twi_master_write(uint8_t * data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    while (true)
    {
        ...
        if (timeout == 0 || NRF_TWI1->EVENTS_ERROR != 0)
        {
            ...
            (void)twi_master_init(m_twi_scl_pin, m_twi_sda_pin);
...
static bool twi_master_clear_bus(void)
{
    ...
    clk_pin_config = \
        NRF_GPIO->PIN_CNF[m_twi_scl_pin];
    NRF_GPIO->PIN_CNF[m_twi_scl_pin] =      \
    ...
    data_pin_config = \
        NRF_GPIO->PIN_CNF[m_twi_sda_pin];
    NRF_GPIO->PIN_CNF[m_twi_sda_pin] =       \
        (GPIO_PIN_CNF_SENSE_Disabled  << GPIO_PIN_CNF_SENSE_Pos) \
      | (GPIO_PIN_CNF_DRIVE_S0D1    << GPIO_PIN_CNF_DRIVE_Pos)   \
      | (GPIO_PIN_CNF_PULL_Pullup   << GPIO_PIN_CNF_PULL_Pos)    \
    ...
    NRF_GPIO->PIN_CNF[m_twi_scl_pin] = clk_pin_config;
    NRF_GPIO->PIN_CNF[m_twi_sda_pin]  = data_pin_config;
...
bool twi_master_init(uint32_t clk_pin, uint32_t sda_pin)
{
    ...
    */
    m_twi_scl_pin = clk_pin;
    m_twi_sda_pin = sda_pin;
    ...
    NRF_TWI1->EVENTS_RXDREADY = 0;
    NRF_TWI1->EVENTS_TXDSENT  = 0;
    NRF_TWI1->PSELSCL         = m_twi_scl_pin;
    NRF_TWI1->PSELSDA         = m_twi_sda_pin;
