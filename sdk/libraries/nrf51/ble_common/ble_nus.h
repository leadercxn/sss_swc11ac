...
typedef enum
{
    BLE_NUS_EVT_DATA_WRITE,
    BLE_NUS_EVT_LONG_DATA_WRITE,
    BLE_NUS_EVT_PASSWORD_WRITE,
} ble_nus_evt_type_t;

#define NUS_BASE_UUID {0xE4,0xE0,0xCA,0x0C,0xA3,0x50,0x4A,0x83,0xA2,0x1B,0x4E,0x7A,0x29,0x69,0xAE,0xDE }

#define BLE_UUID_NUS_SERVICE 0x0500                      /**< The UUID of the Nordic UART Service. */
...
typedef void (*ble_nus_data_handler_t) (ble_nus_t*           p_nus,
                                        ble_nus_evt_type_t   evt_type,
                                        uint16_t             value_handle,
                                        uint8_t*             p_data,
                                        uint16_t             length);

typedef void (*ble_nus_hvc_handler_t) (ble_nus_t * p_nus, ble_gatts_char_handles_t   char_handle);

typedef void (*ble_nus_ind_handler_t) (bool status);
...
typedef struct
{
    ...
    ble_nus_hvc_handler_t  hvc_handler;
} ble_nus_init_t;
...
struct ble_nus_s
{
    ...
    ble_nus_data_handler_t   data_handler;            /**< Event handler to be called for handling received data. */
    ble_nus_hvc_handler_t    hvc_handler;
    ble_nus_ind_handler_t    ind_handler;
};
