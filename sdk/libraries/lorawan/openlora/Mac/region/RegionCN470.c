...
/*!
 * Get SymbTimeout of Cad data packet
 */
...
static uint16_t GetSymbTimeoutCad( int8_t dr )
{
...
void RegionCN470GetPhyParam( GetPhyParams_t* getPhy )
{
    switch ( getPhy->Attribute )
    {
        ...
        {
            getPhy->Param.Value = CN470_DEFAULT_MAX_EIRP;
            break;
        }
        ...
        {
            getPhy->Param.Value = (uint32_t)CN470_RSSI_FREE_TH;
            break;
        }
        case PHY_DEF_LBT_RETRYTIMES:
        {
            getPhy->Param.Value = (uint8_t)CN470_LBT_RETRY_TIMES;
            break;
        }
        default:
        {
...
bool RegionCN470CadRxConfig( RxConfigParams_t* rxConfig )
{
    uint8_t dr = rxConfig->Datarate;
    uint16_t symbTimeout = GetSymbTimeout( dr );
    int8_t phyDr = DataratesCN470[dr];
    uint32_t frequency = CN470_FIRST_RX1_CHANNEL + rxConfig->Channel * CN470_STEPWIDTH_RX1_CHANNEL;;
    uint32_t preambleLen = PreambleLenOfCad[dr];

    Radio.SetChannel( frequency );
    Radio.SetRxConfig( MODEM_LORA, 0, phyDr, 1, 0, preambleLen, symbTimeout, false, 0, false, 0, 0, false, rxConfig->RxContinuous );
...
bool RegionCN470RxConfig( RxConfigParams_t* rxConfig, int8_t* datarate )
{
    ...
    if (rxConfig->Type == true)
    {
        symbTimeout = GetSymbTimeoutCad( dr );
    }
    else
    {
...
bool RegionCN470TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, uint32_t* txTimeOnAir )
{
...
bool RegionCN470NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, uint32_t* time )
{
    ...
    {
        // Update bands Time OFF //wanghuayuan
        nextTxDelay = RegionCommonUpdateBandTimeOff( nextChanParams->DutyCycleEnabled, Bands, CN470_MAX_NB_BANDS );
    ...
    if ( nbEnabledChannels > 0 )
    {
        if (nextChanParams->LbtEnabled)
        {
            for ( uint8_t  i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < nbEnabledChannels; i++ )
            {
                ...
                while ( ELAPSED_TIME_GET( carrierSenseTime ) < CN470_CARRIER_SENSE_TIME )
                {
