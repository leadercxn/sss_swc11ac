 ...
 * LoRaMAC channels
 */
static ChannelParams_t Channels[AU915_HYBRID_MAX_NB_CHANNELS];

/*!
 * LoRaMac bands
 */
static Band_t Bands[AU915_HYBRID_MAX_NB_BANDS] =
{
    AU915_HYBRID_BAND0
};
...
{
    int8_t datarate = DatarateOffsetsAU915_Hybrid[dr][drOffset];
...
static uint8_t CountNbOfEnabledChannels( uint8_t datarate, uint16_t* channelsMask, ChannelParams_t* channels, Band_t* bands, uint8_t* enabledChannels, uint8_t* delayTx )
{
    ...
    for ( uint8_t i = 0, k = 0; i < AU915_HYBRID_MAX_NB_CHANNELS; i += 16, k++ )
    {
        for ( uint8_t j = 0; j < 16; j++ )
        {
            if ( ( channelsMask[k] & ( 1 << j ) ) != 0 )
            {
                ...
                {   // Check if the channel is enabled
                    trace_verbose("Frequency = 0\r\n");
                    continue;
                }
                if ( RegionCommonValueInRange( datarate, channels[i + j].DrRange.Fields.Min,
                                               channels[i + j].DrRange.Fields.Max ) == false )
                {   // Check if the current channel selection supports the given datarate
                    trace_verbose("invalide datarate = %d, max = %d, min = %d\r\n", datarate,
                                  channels[i + j].DrRange.Fields.Max, channels[i + j].DrRange.Fields.Min);
                    continue;
                }
                if ( bands[channels[i + j].Band].TimeOff > 0 )
                {   // Check if the band is available for transmission
                    delayTransmission++;
                    trace_verbose("band not available\r\n");
                    continue;
                }
    ...
    trace_verbose("delayTransmission = %d\r\n", delayTransmission);
...
void RegionAU915HybridGetPhyParam( GetPhyParams_t* getPhy )
{
    switch ( getPhy->Attribute )
    {
        case PHY_MIN_DR:
        {
            getPhy->Param.Value = AU915_HYBRID_TX_MIN_DATARATE;
            break;
        }
        case PHY_DEF_TX_DR:
        {
            getPhy->Param.Value = AU915_HYBRID_DEFAULT_DATARATE;
            break;
        }
        case PHY_DEF_TX_POWER:
        {
            getPhy->Param.Value = AU915_HYBRID_DEFAULT_TX_POWER;
            break;
        }
        case PHY_MAX_PAYLOAD:
        {
            getPhy->Param.Value = MaxPayloadOfDatarateAU915_Hybrid[getPhy->Datarate];
            break;
        }
        case PHY_MAX_PAYLOAD_REPEATER:
        {
            getPhy->Param.Value = MaxPayloadOfDatarateRepeaterAU915_Hybrid[getPhy->Datarate];
            break;
        }
        case PHY_DUTY_CYCLE:
        {
            getPhy->Param.Value = AU915_HYBRID_DUTY_CYCLE_ENABLED;
            break;
        }
        case PHY_MAX_RX_WINDOW:
        {
            getPhy->Param.Value = AU915_HYBRID_MAX_RX_WINDOW;
            break;
        }
        case PHY_RECEIVE_DELAY1:
        {
            getPhy->Param.Value = AU915_HYBRID_RECEIVE_DELAY1;
            break;
        }
        case PHY_RECEIVE_DELAY2:
        {
            getPhy->Param.Value = AU915_HYBRID_RECEIVE_DELAY2;
            break;
        }
        case PHY_JOIN_ACCEPT_DELAY1:
        {
            getPhy->Param.Value = AU915_HYBRID_JOIN_ACCEPT_DELAY1;
            break;
        }
        case PHY_JOIN_ACCEPT_DELAY2:
        {
            getPhy->Param.Value = AU915_HYBRID_JOIN_ACCEPT_DELAY2;
            break;
        }
        case PHY_MAX_FCNT_GAP:
        {
            getPhy->Param.Value = AU915_HYBRID_MAX_FCNT_GAP;
            break;
        }
        case PHY_ACK_TIMEOUT:
        {
            getPhy->Param.Value = ( AU915_HYBRID_ACKTIMEOUT + randr( -AU915_HYBRID_ACK_TIMEOUT_RND, AU915_HYBRID_ACK_TIMEOUT_RND ) );
            break;
        }
        case PHY_DEF_DR1_OFFSET:
        {
            getPhy->Param.Value = AU915_HYBRID_DEFAULT_RX1_DR_OFFSET;
            break;
        }
        case PHY_DEF_RX2_FREQUENCY:
        {
            getPhy->Param.Value = AU915_HYBRID_RX_WND_2_FREQ;
            break;
        }
        case PHY_DEF_RX2_DR:
        {
            getPhy->Param.Value = AU915_HYBRID_RX_WND_2_DR;
            break;
        }
        ...
        {
            getPhy->Param.Value = AU915_HYBRID_MAX_NB_CHANNELS;
            break;
        }
        case PHY_CHANNELS:
        {
        ...
        {
            getPhy->Param.Value = (uint32_t)AU915_HYBRID_RSSI_FREE_TH;
            break;
        }
        case PHY_DEF_LBT_RETRYTIMES:
        {
            getPhy->Param.Value = (uint8_t)AU915_HYBRID_LBT_RETRY_TIMES;
            break;
        }
        default:
        {
...
void RegionAU915HybridInitDefaults( InitType_t type )
{
    switch ( type )
    {
        case INIT_TYPE_INIT:
        {
            // Channels
            // 125 kHz channels
            for ( uint8_t i = 0; i < AU915_HYBRID_MAX_NB_CHANNELS - 8; i++ )
            {
            ...
            // 500 kHz channels
            for ( uint8_t i = AU915_HYBRID_MAX_NB_CHANNELS - 8; i < AU915_HYBRID_MAX_NB_CHANNELS; i++ )
            {
                Channels[i].Frequency = 915900000 + ( i - ( AU915_HYBRID_MAX_NB_CHANNELS - 8 ) ) * 1600000;
...
bool RegionAU915HybridVerify( VerifyParams_t* verify, PhyAttribute_t phyAttribute )
{
    switch ( phyAttribute )
    {
        case PHY_TX_DR:
        {
            return RegionCommonValueInRange( verify->Datarate, AU915_HYBRID_TX_MIN_DATARATE, AU915_HYBRID_TX_MAX_DATARATE );
        }
        case PHY_DEF_TX_DR:
        {
        ...
        case PHY_TX_POWER:
        {
            ...
            return RegionCommonValueInRange( verify->TxPower, AU915_HYBRID_MAX_TX_POWER, AU915_HYBRID_MIN_TX_POWER );
        }
        case PHY_DUTY_CYCLE:
        {
            return AU915_HYBRID_DUTY_CYCLE_ENABLED;
        }
        case PHY_NB_JOIN_TRIALS:
        {
...
bool RegionAU915HybridAdrNext( AdrNextParams_t* adrNext, int8_t* drOut, int8_t* txPowOut, uint32_t* adrAckCounter )
{
    ...
    *adrAckCounter = adrNext->AdrAckCounter;
    ...
    {
        if ( datarate == AU915_HYBRID_TX_MIN_DATARATE )
        {
            *adrAckCounter = 0;
            adrAckReq = false;
        }
        ...
        {
            if ( adrNext->AdrAckCounter >= AU915_HYBRID_ADR_ACK_LIMIT )
            {
                adrAckReq = true;
            }
            else
            ...
            if ( adrNext->AdrAckCounter >= ( AU915_HYBRID_ADR_ACK_LIMIT + AU915_HYBRID_ADR_ACK_DELAY ) )
            {
                if ( ( adrNext->AdrAckCounter % AU915_HYBRID_ADR_ACK_DELAY ) == 0 )
                {
                    if ( txPower == AU915_HYBRID_MAX_TX_POWER )
                    {
                        if ( ( datarate > AU915_HYBRID_TX_MIN_DATARATE ) && ( datarate == DR_8 ) )
                        {
                            datarate = DR_4;
                        }
                        else if ( datarate > AU915_HYBRID_TX_MIN_DATARATE )
                        {
                            datarate--;
                        }
                    ...
                    {
                        txPower = AU915_HYBRID_MAX_TX_POWER;
                    }
                }
            }
        }
...
bool RegionAU915HybridRxConfig( RxConfigParams_t* rxConfig, int8_t* datarate )
{
    ...
    if ( rxConfig->Window == 0 )
    {
        ...
        frequency = AU915_HYBRID_FIRST_RX1_CHANNEL + ( rxConfig->Channel % 8 ) * AU915_HYBRID_STEPWIDTH_RX1_CHANNEL;
    }
    symbTimeout = GetSymbTimeout( dr );
    bandwidth = GetBandwidth( dr );
    // Read the physical datarate from the datarates table
    phyDr = DataratesAU915_Hybrid[dr];
    ...
    {
        maxPayload = MaxPayloadOfDatarateRepeaterAU915_Hybrid[dr];
    }
    else
    {
        maxPayload = MaxPayloadOfDatarateAU915_Hybrid[dr];
    }
...
{
    int8_t phyDr = DataratesAU915_Hybrid[txConfig->Datarate];
    int8_t txPowerLimited = LimitTxPower( txConfig->TxPower, txConfig->Datarate, ChannelsMask );
    uint32_t bandwidth = GetBandwidth( txConfig->Datarate );
    int8_t phyTxPower = 0;

    phyTxPower = TxPowersAU915_Hybrid[txPowerLimited];
...
uint8_t RegionAU915HybridLinkAdrReq( LinkAdrReqParams_t* linkAdrReq, int8_t* drOut, int8_t* txPowOut, uint8_t* nbRepOut, uint8_t* nbBytesParsed )
{
    ...
    // Verify datarate
    if ( RegionCommonChanVerifyDr( AU915_HYBRID_MAX_NB_CHANNELS, channelsMask, linkAdrParams.Datarate, AU915_HYBRID_TX_MIN_DATARATE, AU915_HYBRID_TX_MAX_DATARATE, Channels  ) == false )
    {
        status &= 0xFD; // Datarate KO
    }

    // Verify tx power
    if ( RegionCommonValueInRange( linkAdrParams.TxPower, AU915_HYBRID_MAX_TX_POWER, AU915_HYBRID_MIN_TX_POWER ) == 0 )
    {
        // Verify if the maximum TX power is exceeded
        if ( AU915_HYBRID_MAX_TX_POWER > linkAdrParams.TxPower )
        {   // Apply maximum TX power. Accept TX power.
            linkAdrParams.TxPower = AU915_HYBRID_MAX_TX_POWER;
        }
        else
        {
...
uint8_t RegionAU915HybridRxParamSetupReq( RxParamSetupReqParams_t* rxParamSetupReq )
{
    ...
    if ( ( Radio.CheckRfFrequency( freq ) == false ) ||
            ( freq < AU915_HYBRID_FIRST_RX1_CHANNEL ) ||
            ( freq > AU915_HYBRID_LAST_RX1_CHANNEL ) ||
            ( ( ( freq - ( uint32_t ) AU915_HYBRID_FIRST_RX1_CHANNEL ) % ( uint32_t ) AU915_HYBRID_STEPWIDTH_RX1_CHANNEL ) != 0 ) )
    {
        status &= 0xFE; // Channel frequency KO
    }

    // Verify datarate
    if ( RegionCommonValueInRange( rxParamSetupReq->Datarate, AU915_HYBRID_RX_MIN_DATARATE, AU915_HYBRID_RX_MAX_DATARATE ) == false )
    {
        status &= 0xFD; // Datarate KO
    }
    ...
    if ( RegionCommonValueInRange( rxParamSetupReq->DrOffset, AU915_HYBRID_MIN_RX1_DR_OFFSET, AU915_HYBRID_MAX_RX1_DR_OFFSET ) == false )
    {
...
void RegionAU915HybridCalcBackOff( CalcBackOffParams_t* calcBackOff )
{
    ...
    trace_verbose("RegionAU915HybridCalcBackOff\r\n");
    if ( calcBackOff->Joined == false )
    {
        ...
        trace_verbose("TimeOff = %d\r\n", Bands[Channels[channel].Band].TimeOff);
    }
    else
    {
        Bands[Channels[channel].Band].TimeOff = 0;
        trace_verbose("TimeOff = 0\r\n");
    }
}
...
bool RegionAU915HybridNextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, uint32_t* time )
{
    ...
    uint8_t nbEnabledChannels = 0;
    uint8_t delayTx = 0;
    uint8_t enabledChannels[AU915_HYBRID_MAX_NB_CHANNELS] = { 0 };
    ...
    elapsed = ELAPSED_TIME_GET( nextChanParams->LastAggrTx );
    if ( nextChanParams->AggrTimeOff <=  elapsed)
    {
        // Update bands Time OFF
        nextTxDelay = RegionCommonUpdateBandTimeOff( nextChanParams->DutyCycleEnabled, Bands, AU915_HYBRID_MAX_NB_BANDS );

        // Search how many channels are enabled
        nbEnabledChannels = CountNbOfEnabledChannels( nextChanParams->Datarate,
                            ChannelsMask, Channels,
                            Bands, enabledChannels, &delayTx );
        trace_verbose("nbEnabledChannels = %d\r\n", nbEnabledChannels);
    }
    else
    {
        delayTx++;
    ...
    if ( nbEnabledChannels > 0 )
    {
        if (nextChanParams->LbtEnabled)
        {
            for ( uint8_t  i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < nbEnabledChannels; i++ )
            {
                ...
                while ( ELAPSED_TIME_GET( carrierSenseTime ) < AU915_HYBRID_CARRIER_SENSE_TIME )
                {
                ...
                if ( channelFree == true )
                {
                    ...
                    *channel = channelNext;
                    ...
                    RegionCommonChanDisable( ChannelsMaskRemaining, *channel, AU915_HYBRID_MAX_NB_CHANNELS - 8 );
