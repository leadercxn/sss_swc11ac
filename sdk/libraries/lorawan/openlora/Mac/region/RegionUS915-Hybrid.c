        ...
        for ( uint8_t j = 0; j < 16; j++ )
        {
            if ( ( channelsMask[k] & ( 1 << j ) ) != 0 )
            {
                ...
                {   // Check if the current channel selection supports the given datarate
                    trace_verbose("RegionCommonValueInRange\r\n");
                    continue;
                }
...
void RegionUS915HybridGetPhyParam( GetPhyParams_t* getPhy )
{
    switch ( getPhy->Attribute )
    {
        ...
        {
            getPhy->Param.Value = US915_HYBRID_DEFAULT_MAX_EIRP;
            break;
        }
        ...
        {
            getPhy->Param.Value = (uint32_t)US915_HYBRID_RSSI_FREE_TH;
            break;
        }
        case PHY_DEF_LBT_RETRYTIMES:
        {
            getPhy->Param.Value = (uint8_t)US915_HYBRID_LBT_RETRY_TIMES;
            break;
        }
        default:
        {
...
//            ChannelsDefaultMask[4] = 0x0001;
            ChannelsDefaultMask[4] = 0x0000; //wanghuayuan
...
bool RegionUS915HybridTxConfig( TxConfigParams_t* txConfig, int8_t* txPower, uint32_t* txTimeOnAir )
{
...
bool RegionUS915HybridNextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, uint32_t* time )
{
    ...
    trace_verbose("RegionUS915HybridNextChannel\r\n");
    ...
    if ( nbEnabledChannels > 0 )
    {
        if (nextChanParams->LbtEnabled)
        {
            for ( uint8_t  i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < nbEnabledChannels; i++ )
            {
                ...
                while ( ELAPSED_TIME_GET( carrierSenseTime ) < US915_HYBRID_CARRIER_SENSE_TIME )
                {
