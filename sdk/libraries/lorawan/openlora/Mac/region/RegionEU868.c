...
void RegionEU868GetPhyParam( GetPhyParams_t* getPhy )
{
    switch ( getPhy->Attribute )
    {
        ...
        {
            getPhy->Param.Value = EU868_DEFAULT_MAX_EIRP;
            break;
        }
        ...
        {
            getPhy->Param.Value = (uint32_t)EU868_RSSI_FREE_TH;
            break;
        }
        case PHY_DEF_LBT_RETRYTIMES:
        {
            getPhy->Param.Value = (uint8_t)EU868_LBT_RETRY_TIMES;
            break;
        }
        default:
        {
...
bool RegionEU868TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, uint32_t* txTimeOnAir )
{
    RadioModems_t modem;
            ...
            for ( uint8_t i = 0; i < EU868_MAX_NB_CHANNELS; i++ )
            {
                ...
                else
                {
                    ...
                    {   // Trying to enable an undefined channel
                        status &= 0xFE; // Channel mask KO
                        trace_warn("Trying to enable an undefined channel\r\n");
                    }
                }
            }
        }
...
bool RegionEU868NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, uint32_t* time )
{
    ...
    if ( nbEnabledChannels > 0 )
    {
        if (nextChanParams->LbtEnabled)
        {
            for ( uint8_t  i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < nbEnabledChannels; i++ )
            {
                ...
                while ( ELAPSED_TIME_GET( carrierSenseTime ) < EU868_CARRIER_SENSE_TIME )
                {
...
LoRaMacStatus_t RegionEU868ChannelAdd( ChannelAddParams_t* channelAdd )
{
    ...
    trace_verbose("ch %d = %u\r\n", id, channelAdd->NewChannel->Frequency);
