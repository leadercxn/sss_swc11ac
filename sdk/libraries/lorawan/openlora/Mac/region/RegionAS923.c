...
static bool VerifyTxFreq( uint32_t freq )
{
    ...
    // wanghuayuan
    if ( ( freq < 920000000 ) || ( freq > 928000000 ) )
    {
        return false;
    }
    return true;
...
void RegionAS923GetPhyParam( GetPhyParams_t* getPhy )
{
    switch ( getPhy->Attribute )
    {
        case PHY_MIN_DR:
        {
            ...
            {
                getPhy->Param.Value = AS923_TX_Dwell1_MIN_DATARATE;
            }
            break;
        }
        case PHY_DEF_TX_DR:
        ...
        {
            getPhy->Param.Value = (uint32_t)AS923_RSSI_FREE_TH;
            break;
        }
        case PHY_DEF_LBT_RETRYTIMES:
        {
            getPhy->Param.Value = (uint8_t)AS923_LBT_RETRY_TIMES;
            break;
        }
        default:
        {
...
bool RegionAS923TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, uint32_t* txTimeOnAir )
{
    RadioModems_t modem;
...
bool RegionAS923NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, uint32_t* time )
{
    ...
    if ( nbEnabledChannels > 0 )
    {
        if (nextChanParams->LbtEnabled)
        {
            for ( uint8_t  i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < AS923_MAX_NB_CHANNELS; i++ )
            {
                ...
                while ( ELAPSED_TIME_GET( carrierSenseTime ) < AS923_CARRIER_SENSE_TIME )
                {
