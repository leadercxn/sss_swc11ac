...
void RegionIN865GetPhyParam( GetPhyParams_t* getPhy )
{
    switch ( getPhy->Attribute )
    {
        ...
        {
            getPhy->Param.Value = IN865_DEFAULT_MAX_EIRP;
            break;
        }
        ...
        {
            getPhy->Param.Value = (uint32_t)IN865_RSSI_FREE_TH;
            break;
        }
        case PHY_DEF_LBT_RETRYTIMES:
        {
            getPhy->Param.Value = (uint8_t)IN865_LBT_RETRY_TIMES;
            break;
        }
        default:
        {
...
bool RegionIN865TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, uint32_t* txTimeOnAir )
{
    RadioModems_t modem;
...
bool RegionIN865NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, uint32_t* time )
{
    ...
    if ( nbEnabledChannels > 0 )
    {
        if (nextChanParams->LbtEnabled)
        {
            for ( uint8_t  i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < nbEnabledChannels; i++ )
            {
                ...
                while ( ELAPSED_TIME_GET( carrierSenseTime ) < IN865_CARRIER_SENSE_TIME )
                {
