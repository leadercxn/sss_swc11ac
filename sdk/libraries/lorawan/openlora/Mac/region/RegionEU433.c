...
static bool VerifyTxFreq( uint32_t freq )
{
    ...
    // wanghuayuan
    if ( ( freq < 433000000 ) || ( freq > 440000000 ) )
    {
        return false;
    }
    return true;
...
void RegionEU433GetPhyParam( GetPhyParams_t* getPhy )
{
    switch ( getPhy->Attribute )
    {
        ...
        {
            getPhy->Param.Value = EU433_DEFAULT_MAX_EIRP;
            break;
        }
        ...
        {
            getPhy->Param.Value = (uint32_t)EU433_RSSI_FREE_TH;
            break;
        }
        case PHY_DEF_LBT_RETRYTIMES:
        {
            getPhy->Param.Value = (uint8_t)EU433_LBT_RETRY_TIMES;
            break;
        }
        default:
        {
...
bool RegionEU433TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, uint32_t* txTimeOnAir )
{
    RadioModems_t modem;
...
bool RegionEU433NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, uint32_t* time )
{
    ...
    if ( nbEnabledChannels > 0 )
    {
        if (nextChanParams->LbtEnabled)
        {
            for ( uint8_t  i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < nbEnabledChannels; i++ )
            {
                ...
                while ( ELAPSED_TIME_GET( carrierSenseTime ) < EU433_CARRIER_SENSE_TIME )
                {
