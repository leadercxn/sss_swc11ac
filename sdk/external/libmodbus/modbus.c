...
void _error_print(modbus_t *ctx, const char *context)
{
    if (ctx->debug) {
        if (context != NULL) {
            modbus_debug("[ERROR] [%s]: %s\n", context, modbus_strerror(errno));
        } else {
            modbus_debug("[ERROR] %s\n", modbus_strerror(errno));
        }
    }
}
#if 1
                                                  ...
                                                  msg_type_t msg_type)
{
    ...
    if (msg_type == MSG_INDICATION) {     // 用在服务器端，一直等待，直到有客户端发起请求
                              ...
                              uint8_t *rsp, int rsp_length)
{
    ...
    if ((rsp_length == rsp_length_computed ||
        ...
        if (function != req[offset]) {
            if (ctx->debug) {
                modbus_debug("Received function not corresponding to the request (0x%X != 0x%X)\n",
                        function, req[offset]);
            }
            if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) {
                _sleep_response_timeout(ctx);
        ...
        if (req_nb_value == rsp_nb_value) {
            ...
            if (ctx->debug) {
                modbus_debug("Quantity not corresponding to the request (%d != %d)\n", rsp_nb_value, req_nb_value);
            }
...
/* Get the timeout interval used by the server to wait for an indication from a client */
int modbus_get_timeout(modbus_t *ctx, uint32_t *to_msec)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
int modbus_set_timeout(modbus_t *ctx, uint32_t to_msec)
{
    /* Indication timeout can be disabled when both values are zero */
    if (ctx == NULL || to_msec > 999999) {
        errno = EINVAL;
        return -1;
    }
