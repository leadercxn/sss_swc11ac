...
static int _modbus_rtu_send(modbus_t *ctx, uint8_t *req, uint16_t req_length)
{
    modbus_rtu_port_send(req, req_length);
    return req_length;
}
...
static int _modbus_rtu_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length)
{
    return modbus_rtu_port_recv(rsp, rsp_length, ctx->timeout);
}
...
{
    modbus_rtu_t *ctx_rtu = ctx->backend_data;
    modbus_rtu_port_connect(ctx_rtu->baud);
    return 0;
}
...
int modbus_rtu_set_serial_mode(modbus_t *ctx, int mode)
{
    ...
    if (ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_RTU) {
        ...
        if (ctx->debug) {
            modbus_debug("This function isn't supported on your platform\n");
        }
        return -1;
    }
    errno = EINVAL;
                         ...
                         int stop_bit)
{
    ...
    if (baud == 0) {
        if (ctx->debug) {
            modbus_debug("The baud rate value must not be zero\n");
        }
        errno = EINVAL;
        return NULL;
    }
