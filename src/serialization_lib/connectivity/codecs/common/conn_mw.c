...
static conn_mw_handler_t conn_mw_handler_get(uint8_t opcode)
{
    ...
    for (i = 0; i < conn_mw_item_len; i++)              //遍历操作表，找出匹配的操作，返回对应的函数指针
    {
                         ...
                         uint32_t * const      p_tx_buf_len)
{
    ...
    uint8_t           opcode   = p_rx_buf[SER_CMD_OP_CODE_POS];                 //选怎样的操作

    fp_handler = conn_mw_handler_get(opcode);

    if (fp_handler)
    {
        err_code = fp_handler(p_rx_buf, rx_buf_len, p_tx_buf, p_tx_buf_len);    //执行函数
    }
    else
    {
        err_code = NRF_ERROR_NOT_SUPPORTED;                                     //不支持该命令
    }
