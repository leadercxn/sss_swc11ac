...
static void phy_events_handler(ser_phy_evt_t phy_event)
{
    ...
    LOG_I("transport_ event : %s\n" , ser_phy_event_log_str(phy_event.evt_type) );
    switch (phy_event.evt_type)                                     //根据物理层的事件进行相对应的事件处理
    {
        ...
        case SER_PHY_EVT_RX_BUF_REQUEST:                            //长生接收请求之后，就申请接收区
        {
            ...
            if (phy_event.evt_params.rx_buf_request.num_of_bytes <= sizeof (m_rx_buffer))           //申请的接收区在合理范围之内
            {
                if (HAL_TRANSP_RX_STATE_IDLE == m_rx_state)                                         //接收空闲状态
                {
                    m_events_handler(hal_transp_event);
                    err_code = ser_phy_rx_buf_set(m_rx_buffer);                                     //设置接收缓冲区
                    APP_ERROR_CHECK(err_code);
                    m_rx_state = HAL_TRANSP_RX_STATE_RECEIVING;                                     //切换到正在接收的状态
                }
            ...
            else                                                                                    //申请的接收区域过大，把接收的数据丢入垃圾区，并产生丢失事件
            {
        ...
        case SER_PHY_EVT_RX_PKT_RECEIVED:                                                           //底层接收完成，准备传到顶层
        {
        ...
        {
            //LOG_E("SER_PHY_EVT_RX_OVERFLOW_ERROR ========> \n");
            /* Generate the event to an upper layer. */
        ...
        {
            //LOG_E("SER_PHY_EVT_TX_OVERREAD_ERROR ========> \n");
            /* Generate the event to an upper layer. */
            hal_transp_event.evt_type                        = SER_HAL_TRANSP_EVT_PHY_ERROR;
            hal_transp_event.evt_params.phy_error.error_type =
        ...
        case SER_PHY_EVT_HW_ERROR:                                                                  //硬件层错误
        {
            //LOG_E("SER_PHY_EVT_HW_ERROR ========> \n");
            /* Generate the event to an upper layer. */
            hal_transp_event.evt_type                        = SER_HAL_TRANSP_EVT_PHY_ERROR;
            hal_transp_event.evt_params.phy_error.error_type =
